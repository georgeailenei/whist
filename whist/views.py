...
class WhistView(TemplateView):
    ...
    def get(self, request):
        form = GameForm()
        game = self.controller.load_game() 

        # Players names & Players Obj
        names = game['players_names']
        players = [Player(names[0]), Player(names[1]), Player(names[2]), Player(names[3])]
        

        # Players cards & tricks
        current_cards = game['players_cards']
        current_tricks = game['players_tricks']
        removed_cards = game['removed_players_cards']
        ...
        # Player position & reset if goes above 3;
